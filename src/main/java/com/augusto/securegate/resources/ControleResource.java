...
public class ControleResource {
	...
	@PutMapping(value = "/{id}")
	public ResponseEntity<ControleDTO> update(@PathVariable Integer id, @Valid @RequestBody ControleDTO objDTO){
		Controle newObj = service.update(id, objDTO);
		return ResponseEntity.ok().body(new ControleDTO(newObj));
	}
